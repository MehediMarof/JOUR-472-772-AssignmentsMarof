...
maryland_population |> left_join(county_voters_2020, join_by(COUNTY == county)) |> nrow()
```
...
```{r}
# without nrow
maryland_population |> left_join(county_voters_2020, join_by(COUNTY == county))
```
...
**Answer:** R automatically joined the two tables using the common column `COUNTY`, resulting in 24 rows—one for each Maryland county, including Baltimore City.

```{r}
maryland_population_with_voters <- maryland_population |>
left_join(county_voters_2020, join_by(COUNTY == county))

glimpse(maryland_population_with_voters)
```
...
Prince George’s County has the highest 18+ Black population share (\~60.9%) and about **78.3%** Democrats.

```{r}
maryland_population_with_voters_and_demographics |>
  mutate(
    pct_black_18_plus = 100 * (pop_black / pop_18_over),
    pct_dems          = 100 * (dem / total)
  ) |>
...
**Answer:** Howard County has the highest 18+ Asian population share (\~19.4%) and about **52.4%** Democrats, followed by Montgomery County (\~16% Asian, **61%** Democrats). Compared to Task 10, these counties have smaller Black populations but remain strongly Democratic.
...
**Task** Run the following code to change the datatype of ZCTA5N from numeric to character. What's the datatype of ZCTA5N now? **Answer:** it’s `<chr>`.
