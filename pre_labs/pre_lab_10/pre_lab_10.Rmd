...
**Answer:** It loads a simple-features dataframe with 526 ZIP-code rows. Columns include: OBJECTID, ZIPCODE2, ZIPNAME, EXISTING, ZIPCODE1, Shape_len, Shape_Area, and the geometry column containing MULTIPOLYGON shapes.
...
**Answer:** The output shows a spreadsheet-like table with 526 rows, one per ZIP code. Each row has the ZIP code’s ID, the 11-digit ZIP, the ZIP name, an “EXISTING” code, the 5-digit ZIP, shape length, shape area, and the geometry column with the MULTIPOLYGON map shape.
...
**Answer:** It draws a map of Maryland showing all ZIP code boundaries. Each ZIP code appears as a separate polygon outlined on the state’s shape.
...
**Answer:** The `counties()` call loads a nationwide counties shapefile with 3,235 rows and 19 columns. It includes state and county FIPS codes, county names, classification fields, and a geometry column that stores each county’s polygon shape.
...
**Answer:** It shows a clean map of Maryland divided into its counties. The shapes are much larger and fewer than the previous ZIP-code map. The difference is that the earlier map displayed hundreds of tiny ZIP-code polygons, while this one displays only the county outlines, making the map simpler and easier to read.
...
**Answer:** It layers the two maps together. The county outlines form the base, and the ZIP-code boundaries appear on top of them. You can now see how the smaller ZIP shapes fit inside the larger county shapes.
...
**Answer:** The CSV loads correctly and contains 547 rows and 22 columns. After cleaning and selecting, the table shows each ZIP code and its foreclosure count for March 2023.
...
**Answer:** It produces a shaded ZIP-code map of Maryland where each ZIP is colored by how many foreclosure notices it had in March 2023. Darker areas show higher counts. You can clearly see clusters of high foreclosures in Prince George’s County, parts of Baltimore, and a few other pockets across the state.
...
**Answer:** The code pulls 2021 ACS county-level population data for Maryland and includes the county geometries. It returns a simple-features object with each county’s GEOID, name, population estimate, and its MULTIPOLYGON shape.
...
**Answer:** The code cleans the county population table.
...
**Answer:** The code joins the cleaned county population data with the foreclosure notices table by the county name. It produces a dataset with each county’s population, its foreclosure notice count, the date and type fields, and the county geometry.
...
**Answer:** The code joins the population and foreclosure tables again, calculates the foreclosure rate per 1,000 people, and sorts the counties from highest rate to lowest. It outputs the counties ordered by their foreclosure rate.
...
**Answer:** It calculates the centroid of each county, extracts the X-Y coordinates, attaches county names, and then plots a choropleth map where counties are filled by foreclosure rate. It also adds county name labels at their centroid locations on top of the map.
...
**Answer:** The code draws the Maryland county map again, filling each county by its foreclosure rate, but this time it applies the “magma” color scale, which changes the map’s colors to a yellow-to-purple gradient.
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **Answer:** The map shows foreclosure rates using a log scale. Charles County and Prince George’s County have the highest rates, while most western and eastern counties have much lower rates.
