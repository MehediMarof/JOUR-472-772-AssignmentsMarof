...
pg_schools <- cleaned_payments |>
  filter(
    ...
    str_detect(vendor_name, "PRINCE GEORGES CO PUBLIC") |
    str_detect(vendor_name, "PG COUNTY PUBLIC") |
    str_detect(vendor_name, "P G COUNTY PUBLIC") |
...
A2: 2024 had the highest amount paid to PG county school systems. However, some years are missing.

```{r}
pg_schools |>
  group_by(fiscal_year) |>
  summarise(total_payments = sum(amount, na.rm = TRUE)) |>
...
```{r}
capital_projects_finder <- cleaned_payments |> filter(str_detect(agency-name, "Capital Project")) |> group_by(agency_name) |> summarize( count = n())

umd_vendor_totals <- cleaned_payments |>
  filter(
    str_detect(agency_name,
      "UNIVERSITY OF MARYLAND|UNIVERSITY SYSTEM OF MARYLAND|AID TO UNIVERSITY OF MD MEDICAL SYSTEM|AID TO UNIVERSITY OF MD|UMD|UNIV OF MD"
    ),
    str_detect(agency_name, "CAPITAL")
  ) |>
  group_by(agency_name, vendor_name) |>
